...
### Working Directory
- All commands must be run from within `{backend-folder}/`.
- All git operations must be run from within `{backend-folder}/`.
...
**File path**: `sessions/YYYY-MM-DD/YYYY-MM-DDThh-mm-ss_kotlin-spring-boot-backend-developer_<short-task-slug>.md`
